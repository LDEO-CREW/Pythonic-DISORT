...
def test_3multilayer():
    print()
    print("################################################ Test 3multilayer ##################################################")
    print()
    ######################################### PYDISORT ARGUMENTS #######################################

    tau_arr = np.cumsum(0.5 * np.arange(7))[1:]
    omega_arr = np.linspace(1 - 1e-6, 0.8, 6)  # Reduced from 1 (conservative scattering) which is not implemented
    NQuad = 16
    Leg_coeffs_all = np.linspace(0.9, 0.65, 6)[:, None] ** np.arange(32)[None, :]
    mu0 = 1
    I0 = pi / mu0
    phi0 = pi
    ...
    # Load results from version 4.0.99 of Stamnes' DISORT for comparison
    results = np.load("Stamnes_results/3multilayer_test.npz")
